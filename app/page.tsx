...
type CaseExample = { region: string; matches: RegExp; focus: string; url: string };
...
// Casos privados do acervo do usuário, indexados por impressão/hipótese no Google Drive.
// O texto exibido é intencionalmente anonimizado; o PDF original permanece restrito ao Drive do usuário.
const caseExamples: CaseExample[] = [
  { region: "Crânio", matches: /nasal|neoplásico|neoplasico|osteólise|osteolise|massa|inflamatório|inflamatorio/i, focus: "Processo craniofacial inflamatório e neoplásico", url: "https://drive.google.com/file/d/1GTUMLnvlhu-NEBS_svNeG29jSKJwTjUT/view?usp=drivesdk" },
  { region: "Crânio", matches: /temporomandibular|mandíbula|mandibular|luxação/i, focus: "Subluxação temporomandibular", url: "https://drive.google.com/file/d/1GTxdHd9bQZsz3dOC4skfKkQ7u75oq2NI/view?usp=drivesdk" },
  { region: "Coluna", matches: /espondil|disco|lombossacra|cauda equina|osteofitose/i, focus: "Osteoartrose vertebral e instabilidade lombossacra", url: "https://drive.google.com/file/d/1NxMAvXTQ42IoHQJtgOl191E5AM3asBLb/view?usp=drivesdk" },
  { region: "Coluna", matches: /fratura|luxação|sacroilíaca|sacroiliaca/i, focus: "Trauma pélvico e disjunção sacroilíaca", url: "https://drive.google.com/file/d/10BEuAkzHljVRmnqxnx0DwfpR-XtCpQ9W/view?usp=drivesdk" },
  { region: "Membros", matches: /patelar|patela/i, focus: "Luxação patelar medial", url: "https://drive.google.com/file/d/1Fsw6vb4tOScf1RERocj5PkD-Nq1VpwRf/view?usp=drivesdk" },
  { region: "Membros", matches: /fratura|calo|consolidação|consolidacao|não união|nao uniao|má união|ma uniao|monteggia/i, focus: "Osteossíntese e consolidação de rádio e ulna", url: "https://drive.google.com/file/d/16Bnj2pYr-7exRM4jcs8Yh4IiklHC1Hpg/view?usp=drivesdk" },
  { region: "Membros", matches: /bicipital|tenossinovite|tendinopatia/i, focus: "Tendinopatia bicipital crônica", url: "https://drive.google.com/file/d/1M3qlA2cA7eP0T6IWbqvzV3TIbO_Y1Kel/view?usp=drivesdk" },
  { region: "Membros", matches: /osteoart|osteoartrite|artrite|efusão articular|efusao articular|osteocondrose/i, focus: "Doença articular degenerativa", url: "https://drive.google.com/file/d/1Iih7gR_gu1lbHWMvBCLUyrSSQ69QI7-W/view?usp=drivesdk" },
  { region: "Coxal", matches: /displasia|subluxação|subluxacao|coxofemoral/i, focus: "Displasia coxofemoral com subluxação bilateral", url: "https://drive.google.com/file/d/1s_Y7bINCyq0uERn907cNhMpPl68UIInl/view?usp=drivesdk" },
  { region: "Coxal", matches: /luxação|luxacao/i, focus: "Luxação coxofemoral", url: "https://drive.google.com/file/d/16Wbmy09FK836-5FkSyl97BGihHxgwrSo/view?usp=drivesdk" },
  { region: "Tórax", matches: /broncopatia|bronquite|asma|bronquiectasia/i, focus: "Broncopatia inflamatória e cardiopatia", url: "https://drive.google.com/file/d/194LjJilaNzySQ6VXdV2FNtociNurxcdL" },
  { region: "Tórax", matches: /pneumonia|broncopneumonia|aspirativa|consolidação|consolidacao|hepatização|hepatizacao/i, focus: "Broncopneumonia e padrão alveolar", url: "https://drive.google.com/file/d/15dW3j9J5XcgQjexI-bMagW3F5QVtf5PY" },
  { region: "Tórax", matches: /pneumotórax|pneumotorax/i, focus: "Pneumotórax e retração pulmonar", url: "https://drive.google.com/file/d/1vZOZnrnm6jvBPf6xecjOizTTDwMMuSgr/view?usp=drivesdk" },
  { region: "Tórax", matches: /efusão|efusao|pleural/i, focus: "Efusão pleural e doença pulmonar", url: "https://drive.google.com/file/d/1PDWl-CKbAL5NsQK77UIB1Jkdc-HVvKNS/view?usp=drivesdk" },
  { region: "Tórax", matches: /cardiomegalia|átrio|atrio|ventrículo|ventriculo|cardiopatia|microcardia|pericárdica|pericardica/i, focus: "Cardiomegalia com aumento atrial esquerdo", url: "https://drive.google.com/file/d/1aaDLmWDI6lH_f0i2hGcu9Pm-8K4VF0qt/view?usp=drivesdk" },
  { region: "Tórax", matches: /colapso traqueal|traqueia|traqueal|membrana redundante|estenose/i, focus: "Colapso traqueal", url: "https://drive.google.com/file/d/1E-NvgqMKKUTAw3r6vjpfKf2SjKv_OwZ-/view?usp=drivesdk" },
  { region: "Tórax", matches: /megaesôfago|megaesofago|esôfago|esofago|anel vascular|hérnia de hiato|hernia de hiato/i, focus: "Megaesôfago e broncopneumopatia crônica", url: "https://drive.google.com/file/d/16Sz0izuvfo4uUOsE6iTU3cAlAzPdiBRy/view?usp=drivesdk" },
  { region: "Tórax", matches: /metástase|metastase|neoplásico|neoplasico|nódulo|nodulo/i, focus: "Pneumonia grave com diferencial de metástase", url: "https://drive.google.com/file/d/1MNGcT-AD9KltQq33gYqcdts9AggQp8aw/view?usp=drivesdk" },
  { region: "Tórax", matches: /edema|atelectasia|enfisema|contusão|contusao/i, focus: "Broncopneumopatia com avaliação de padrão pulmonar", url: "https://drive.google.com/file/d/1vzi-zQqdKlMWrt0oDtt-6R9FL9-ys8UY/view?usp=drivesdk" },
  { region: "Abdômen", matches: /nefro|uretero|urolito|cisto|bexiga|cistite|uretral/i, focus: "Nefrolitíase unilateral", url: "https://drive.google.com/file/d/1RYXf9ZMXKdN0f-OpL5mlKbi3I_1xZOgt/view?usp=drivesdk" },
  { region: "Abdômen", matches: /fecaloma|megacólon|megacolon|obstrutivo|intestinal|gástric|gastric|corpo estranho/i, focus: "Fecaloma e retenção urinária", url: "https://drive.google.com/file/d/1ix2u04hJA8xyIq7L47xORrBFf4U6MZVx/view?usp=drivesdk" },
  { region: "Abdômen", matches: /cateter|duplo j|sub|sonda/i, focus: "Cateteres duplo J em trato urinário", url: "https://drive.google.com/file/d/1FDFeB89APSH_2899OqK1VPWCxVvLFnk9/view?usp=drivesdk" },
];
...
function examplesFor(item: Reference) {
  const direct = caseExamples.filter((example) => example.region === item.region && example.matches.test(item.title));
  const related = direct.length ? direct : caseExamples.filter((example) => example.region === item.region);
...
export default function Home() {
  ...
  const [openGuideModules, setOpenGuideModules] = useState<string[]>([]);
...
- [Comentário]`;
  ...
  if (guideOpen) return <main className="thrall-guide-page">
    ...
    <section className="guide-shell">
      ...
      <section className="guide-modules">{thrallStudyGuides.map((guide, index) => { const expanded = openGuideModules.includes(guide.id); return <article className="guide-module" id={guide.id} key={guide.id}><div className="guide-module-head"><span>{String(index + 1).padStart(2, "0")}</span><div><p>{guide.chapters}</p><h2>{guide.title}</h2></div></div><p className="guide-introduction">{guide.introduction}</p><div className="guide-essentials"><p className="section-label">IDEIAS ESSENCIAIS</p><ul>{guide.essentials.map((item) => <li key={item}>{item}</li>)}</ul></div><button className="disclosure-button" type="button" aria-expanded={expanded} aria-controls={`guide-details-${guide.id}`} onClick={() => setOpenGuideModules((current) => expanded ? current.filter((id) => id !== guide.id) : [...current, guide.id])}>{expanded ? "Ocultar passo a passo e armadilhas" : "Ver passo a passo e armadilhas"}<span>{expanded ? "−" : "+"}</span></button>{expanded && <div className="guide-details" id={`guide-details-${guide.id}`}><div><p className="section-label">SEQUÊNCIA DE LEITURA</p><ol>{guide.sequence.map((item) => <li key={item}>{item}</li>)}</ol></div><div><p className="section-label">ARMADILHAS</p><ul>{guide.traps.map((item) => <li key={item}>{item}</li>)}</ul></div></div>}</article>; })}</section>
    </section>
  </main>;
  ...
  if (active) return <main className="writing-mode">
    ...
    <section className="clinical-grid">
      <section className="clinical-reference">
        ...
        <article className="case-examples"><div><p className="section-label">CASOS DO SEU ACERVO</p><h2>Compare com uma radiografia real</h2><p>Abra o PDF para observar as imagens e a redação completa do caso. Os arquivos continuam privados no seu Drive.</p></div><div className="case-example-list">{activeExamples.length ? activeExamples.map((example) => <a key={example.url} href={example.url} target="_blank" rel="noopener noreferrer"><span>{example.direct ? "CASO DO MESMO TEMA" : "CASO CORRELATO"}</span><b>{example.focus}</b><small>Abrir radiografia e laudo ↗</small></a>) : <a href={archiveIndexUrl} target="_blank" rel="noopener noreferrer"><span>ACERVO EM EXPANSÃO</span><b>Consultar o índice dos casos radiográficos</b><small>Localizar um caso desta região ↗</small></a>}</div></article>
  ...
  return <main>
    ...
    <section className="learning-layout">
      ...
      <section className="library"><div className="library-head"><div><p className="section-label">FICHAS PARA CONSULTA</p><h2>{region === "Todos" ? "Escolha uma região" : region}</h2></div><label className="search"><span>⌕</span><input aria-label="Buscar modelo" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar alteração ou estrutura" /></label></div><div className="reference-grid">{visible.map((item) => <article className={`learning-card ${item.kind === "pre-laudo" ? "ready-model" : ""}`} key={item.id} role="button" tabIndex={0} aria-label={`Abrir ficha completa: ${item.title}`} onClick={() => openWriter(item)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openWriter(item); } }}>{item.visual && <div className="card-image"><img src={item.visual.src} alt="" /><span>Referência visual</span></div>}<div className="card-meta"><span>{item.kind === "pre-laudo" ? "PRÉ-LAUDO" : item.region.toUpperCase()}</span><small>Modelo + contexto Thrall</small></div><h3>{item.title}</h3><p>{item.model}</p><div className="card-impression"><b>Impressão direta</b><span>{item.impression}</span></div><div className="thrall-badge">THRALL · {getThrallContext(item.title, item.region).chapter}</div><span className="card-open-link">Abrir ficha completa <b>→</b></span></article>)}</div></section>
